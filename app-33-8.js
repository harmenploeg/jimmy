...
let carouselImageX = 0;
let carouselImageY = 0;
let carouselPointerGesture = null;
...
function bindEvents() {
  ...
  carouselFigure.addEventListener("pointerdown", handleCarouselPointerDown);
  carouselFigure.addEventListener("pointermove", handleCarouselPointerMove);
  carouselFigure.addEventListener("pointerup", handleCarouselPointerEnd);
  carouselFigure.addEventListener("pointercancel", handleCarouselPointerEnd);
...
function handleCarouselTouchStart(event) {
  ...
  if (touch && carouselImageScale > 1.01) {
    event.preventDefault();
    carouselGesture = {
      mode: "pan",
      startX: touch.clientX,
      startY: touch.clientY,
      startImageX: carouselImageX,
      startImageY: carouselImageY
    };
    return;
  }
...
function handleCarouselTouchMove(event) {
  ...
  if (carouselGesture?.mode === "pan" && event.touches.length) {
    event.preventDefault();
    const touch = event.touches[0];
    setCarouselImageOffset(
      carouselGesture.startImageX + touch.clientX - carouselGesture.startX,
      carouselGesture.startImageY + touch.clientY - carouselGesture.startY
    );
    return;
  }
...
function handleCarouselPointerDown(event) {
  if (event.pointerType === "touch" || carouselImageScale <= 1.01 || !event.isPrimary) return;

  event.preventDefault();
  carouselPointerGesture = {
  ...
  carouselFigure.setPointerCapture?.(event.pointerId);
}

function handleCarouselPointerMove(event) {
  if (!carouselPointerGesture || event.pointerId !== carouselPointerGesture.pointerId) return;

  event.preventDefault();
  setCarouselImageOffset(
    carouselPointerGesture.startImageX + event.clientX - carouselPointerGesture.startX,
    carouselPointerGesture.startImageY + event.clientY - carouselPointerGesture.startY
  );
}

function handleCarouselPointerEnd(event) {
  if (carouselPointerGesture?.pointerId === event.pointerId) {
    carouselFigure.releasePointerCapture?.(event.pointerId);
  }
  carouselPointerGesture = null;
}
...
function setCarouselImageScale(scale) {
  ...
  clampCarouselImageOffset();
  applyCarouselImageTransform();
  modalImage.classList.toggle("is-zoomed", carouselImageScale > 1.01);
}

function setCarouselImageOffset(x, y) {
  const bounds = getCarouselPanBounds();
  carouselImageX = clamp(x, -bounds.x, bounds.x);
  carouselImageY = clamp(y, -bounds.y, bounds.y);
  applyCarouselImageTransform();
}

function clampCarouselImageOffset() {
  const bounds = getCarouselPanBounds();
  carouselImageX = clamp(carouselImageX, -bounds.x, bounds.x);
  carouselImageY = clamp(carouselImageY, -bounds.y, bounds.y);
}

function getCarouselPanBounds() {
  const width = modalImage.clientWidth || modalImage.getBoundingClientRect().width || 0;
  const height = modalImage.clientHeight || modalImage.getBoundingClientRect().height || 0;
  return {
    x: Math.max(0, (width * (carouselImageScale - 1)) / 2),
    y: Math.max(0, (height * (carouselImageScale - 1)) / 2)
  };
}

function applyCarouselImageTransform() {
  modalImage.style.transform = `translate3d(${carouselImageX}px, ${carouselImageY}px, 0) scale(${carouselImageScale})`;
}
